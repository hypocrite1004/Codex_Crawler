...
- [ ] `localStorage` 저장 방식 유지 여부 결정
- [ ] 토큰 갱신 흐름과 배포 환경 분리 방안 검토
- [ ] `API_URL` 하드코딩 제거 필요성 판단
...
- [ ] staff가 요약/댓글 관리 가능한지 정책 확정
- [ ] 프론트 버튼 노출이 백엔드 권한과 일치하는지 점검
- [ ] 운영자 QA 흐름 문서화
...
### P1-9. 백엔드 테스트 환경 복구
- [ ] PostgreSQL 로컬 실행 조건 정리
- [ ] 테스트 DB 생성 가능 상태 확인
- [ ] `api.tests` 전체 실행 기준선 확보
...
- [ ] 작성자 초안 저장 → 검토 요청
- [ ] 관리자 승인/반려 → 초안 복원
- [ ] 크롤러 preview → crawl → logs/runs/items
- [ ] CVE 목록 → 상세 → 관련 게시글 → 상세 페이지
