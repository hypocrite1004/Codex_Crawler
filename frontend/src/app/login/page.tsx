...
export default function LoginPage() {
    ...
    const expiredMessage =
        typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('expired') === '1'
            ...
            : '';
    const displayError = error || expiredMessage;
    ...
    return (
        <div className="container" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
            <div className="glass-panel" style={{ width: '100%', maxWidth: '400px' }}>
                ...
                <div suppressHydrationWarning style={{ color: '#ef4444', marginBottom: '1rem', textAlign: 'center', minHeight: '1.5rem' }}>
                    {displayError}
                </div>
                ...
                <div style={{ textAlign: 'center', marginTop: '1.5rem', color: 'var(--text-secondary)' }}>
                    Do not have an account? <Link href="/register" style={{ color: 'var(--accent-primary)' }}>Sign up</Link>
                </div>
            </div>
        </div>
    );
