...
export default function ProfilePostsPanel({
  ...
  workflowBusyId: number | null;
  onWorkflowAction: (postId: number, action: 'submit' | 'restore') => void;
}) {
  const postCounts = {
    all: posts.length,
        ...
        {[
          ...
          return (
            <button
              key={item.key}
              type="button"
              onClick={() => onStatusFilterChange(item.key as PostStatus | 'all')}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '8px',
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <div style={{ display: 'grid', gap: '6px' }}>
                  <Link href={`/posts/${post.id}`} style={{ color: 'var(--text-primary)', textDecoration: 'none', fontSize: '1rem', fontWeight: 600 }}>
                    {post.title}
                  </Link>
                ...
                <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
                  {(post.status === 'draft' || post.status === 'rejected') && (
                    <button
                      ...
                      onClick={() => void onWorkflowAction(post.id, 'submit')}
                  ...
                  {(post.status === 'review' || post.status === 'archived') && (
                    <button
                      ...
                      disabled={workflowBusyId === post.id}
                      onClick={() => void onWorkflowAction(post.id, 'restore')}
                      style={{ padding: '0.5rem 0.9rem' }}
                    >
